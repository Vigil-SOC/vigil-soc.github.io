...
|---|---|---|---|
| **Plain values** | `values.yaml` (or `--set`) | Local dev, one-off demos | [HELM.md — Secrets](/docs/helm/#secrets) |
| **Pre-created Secret** (`secrets.existingSecret`) | Kubernetes | You already manage secrets out-of-band | [HELM.md — Secrets](/docs/helm/#secrets) |
| **ExternalSecrets Operator** (`secrets.externalSecret.enabled`) | AWS SM / Vault / GCP SM | Secrets already in an external store | [HELM.md — Secrets](/docs/helm/#secrets) |
...
`VIRUSTOTAL_API_KEY`, etc. See [env.example](https://github.com/Vigil-SOC/vigil/blob/main/env.example) for the full list.
...
See [docs/examples/sealed-secret.yaml](/docs/examples/sealed-secret.yaml) for a
reference template.
...
See [docs/examples/sops-config.yaml](/docs/examples/sops-config.yaml) for a
reference `.sops.yaml`.
